...
> 给一个博主名字，还你 100 条高质量中文逐字稿。

## 🎯 能干什么

| 你扔进来 | 它还给你 |
|-----------|----------|
| 抖音博主 ID / 昵称 | `{标题}.txt` × N，标点完整、繁转简 |
| 小红书博主主页 | 同上 + 小红书笔记正文 |
| YouTube 频道 | 同上（计划中） |
...
# ffmpeg 也要装：winget install Gyan.FFmpeg  (Windows)

# 3. 注册 TikHub（免费注册，按量付费，¥5 够采 100 条）
#    https://user.tikhub.io → 生成 API Token
...
cp scripts/* blogger-distiller/scripts/utils/
python blogger-distiller/scripts/crawl_douyin.py "博主名" \
  --user-id "MS4wLjAB..." \
  --max-videos 100 \
  -o ./output
```

**30 分钟后**，`./output/` 里全是逐字稿。
...
┌──────────────────────────────────────────────────────────────────┐
│  ./output/                                                       │
│  ├── 001_在婚姻这一块男生从来不慕强.txt                           │
│  ├── 002_大超科普篇之名词解释.txt                                  │
│  ├── ...                                                         │
│  └── transcript_index.json      ← 标题/点赞/评论/时长/路径索引     │
└──────────────────────────────────────────────────────────────────┘
```

### Layer 1 — 元数据采集（TikHub）
...
| 博主 | 平台 | 视频数 | 转写成功率 | 总字数 |
|------|------|--------|-----------|--------|
| 大超说媒（主号） | 抖音 | 19 | 95% | ~6,000 |
| 大超说媒 2 号 | 抖音 | 101 | 100% | ~24,000 |
| **合计** | | **120** | **99.2%** | **~30,000** |
